...
namespace AgileCRM
{
    public class Agile : ICertificatePolicy
    {
        ...
        public static async Task<string> Post(string uri, Object data)
        {
            ...
            httpClient.DefaultRequestHeaders.Authorization = CreateBasicHeader(agileaccount, apiKey);
            httpClient.Timeout = new TimeSpan(0, 0, 5);

            var response = httpClient.PostAsJsonAsync(uri, data);
            string responseString = await response.Result.Content.ReadAsStringAsync();
            return (responseString);
            
        }

        public static async Task<string> CreateContact(AgileContact contact)
        {
            string uri = domainUrl + "contacts/";
            string resp = await Post(uri, contact);
            return (resp);           
        }
